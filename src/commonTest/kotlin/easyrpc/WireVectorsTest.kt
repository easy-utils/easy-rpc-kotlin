...
class WireVectorsTest {
    private val v = parseJson(WIRE_VECTORS_JSON) as Map<*, *>

    private fun hex(b: ByteArray) = b.joinToString("") { byteHex(it) }
    ...
    @Test fun endStream() {
        for (e in arr(v["endStream"])) {
            ...
            if (enc != null && m["bytesHex"] != null) {
                ...
                assertEquals(parseJson(unhex(m["bytesHex"] as String).decodeToString()), parseJson(got.decodeToString()), "${m["name"]} encode")
            }
        }
    }
    ...
    @Test fun unaryError() {
        for (u in arr(v["unaryError"])) {
            ...
            assertEquals(
                parseJson(unhex(m["bytesHex"] as String).decodeToString()),
                parseJson(got.decodeToString()),
                m["name"] as String,
            )
        }
    }
...
internal fun byteHex(b: Byte): String {
    val n = b.toInt() and 0xff
    val digits = "0123456789abcdef"
    return "${digits[n shr 4]}${digits[n and 15]}"
}
